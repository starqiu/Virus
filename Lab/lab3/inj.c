...
int inject_code(DWORD PID)
{
	...
	int TID=0;
	DWORD hproc, hthrd;
	int rstr, rcode, old, numx,nWrite,op;
	DWORD wthunkOfMBA;
	...
	printf("hthrd : 0x%08x\n", hthrd);
	GetExitCodeThread(hthrd, &wthunkOfMBA);
	pthunkOfMBA =(PIMAGE_THUNK_DATA)wthunkOfMBA;
	VirtualProtectEx(hproc,pthunkOfMBA->u1.Function, sizeof(pthunkOfMBA->u1.Function), PAGE_EXECUTE_READWRITE, &op);
	printf("pthunk Of MessageBoxA in the ImportTable of hello.exe: 0x%08x\n", pthunkOfMBA);
	printf("_addr_MessageBoxA : 0x%08x\n", _addr_MessageBoxA);
	*(PDWORD)((PBYTE)_addr_MessageBoxA + off) = pthunkOfMBA->u1.Function;// save the addr of MessageBoxA 
	printf("pthunkOfMBA->u1.Function : 0x%08x\n", pthunkOfMBA->u1.Function);
	//VirtualProtect(pthunkOfMBA, sizeof(pthunkOfMBA), PAGE_EXECUTE_READWRITE, &op);
	pthunkOfMBA->u1.Function= MyMessageBoxA;//redirect to MyMessageBoxA
	printf("pthunkOfMBA->u1.Function : 0x%08x\n", pthunkOfMBA->u1.Function);
    return 0;
}
